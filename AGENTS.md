## プロジェクト概要
- リポジトリ: VitalDB を用いた IOH 予測パイプライン（中央学習 + 連合学習 + ベイズ/アンサンブル比較）。
- 手法: 中央学習、ローカル学習、連合学習（FedAvg）、ベイズ系（FedUAB/pFedBayes）、アンサンブル（FedBE）。
- 主要入口: `scripts/data_download.py` / `scripts/build_dataset.py` / `centralized/train.py` / `centralized/eval.py` / `scripts/train_local.py` / `federated/server.py` / `bayes_federated/feduab_server.py` / `bayes_federated/pfedbayes_server.py` / `fedbe_server.py`
- 大きな成果物は `vitaldb_data/`、`federated_data/`、`runs/`、`outputs_fedbe/`、`outputs_plan/`、`outputs_*`、`*.csv`、`*.npz`、`*.pdf` に配置される。
...
## 実装方針
- 新しい入口を増やすより、既存の `scripts/` と `configs/`、`centralized/`、`federated/`、`bayes_federated/`、`fedbe_server.py` を優先して利用。
