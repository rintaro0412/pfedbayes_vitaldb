...
## 0. 結論（要点）
- クライアント内の分割は train/test=80/20（val なし）を採用している。
- しきい値は固定 0.5 で全手法に統一している。
...
根拠:
- `scripts/build_dataset.py:17,59-68`（分割）
- `common/splits.py:11-26`（分割既定）
- `centralized/eval.py:55-120`（Centralized評価の固定しきい値）
- `scripts/train_local.py:108-335`（Localの固定しきい値）
- `federated/server.py:118-745`（FedAvgの固定しきい値と round×client ログ）
- `bayes_federated/pfedbayes_server.py:260-845`（pFedBayesの固定しきい値/ログ）
- `bayes_federated/feduab_server.py:780-1030`（FedUABの固定しきい値/ログ）
...
- クライアント内の分割は train/test=80/20（val なし）で行う。
...
- `scripts/build_dataset.py:5-18,59-90`（イベント定義・artifact・分割）
...
- `centralized/eval.py:70-162`、`scripts/train_local.py:108-335`、`federated/server.py:118-745`、`bayes_federated/pfedbayes_server.py:260-845`、`bayes_federated/feduab_server.py:780-1030`（しきい値）
...
根拠:
- `README.md:7-15,39-71`
- `scripts/build_dataset.py:64-90`
